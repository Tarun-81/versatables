...
var cryozonic =
{
    ...
    handleCardPaymentResult: function(result, done)
    {
        ...
        {
            cryozonic.refreshPaymentIntent(function()
            {
        ...
        try
        {
            cryozonic.stripeJsV3.confirmPaymentIntent(cryozonic.paymentIntent, params).then(function(confirmResult)
            {
                ...
                if (confirmResult.paymentIntent.status == "requires_capture")
                    cryozonic.handleCardPaymentResult(confirmResult, done);
...
var createStripeToken = function(callback)
{
    ...
    if (newCardRadio && !newCardRadio.checked)
    {
        if (cryozonic.getPaymentIntent() && cryozonic.sourceId)
            ...
            return done(); // Stripe.js is disabled in this case
    }
        ...
        try
        {
            cryozonic.stripeJsV3.createSource(cryozonic.card, { owner: cryozonic.getSourceOwner() }).then(function(result)
            {
                ...
                if (cryozonic.getPaymentIntent())
                    cryozonic.handleCardPayment(result.source, done);
                else
                    return done();
            });
        }
    ...
    else
    {
        ...
        try
        {
            cryozonic.stripeJsV2.card.createToken(cardDetails, function (status, response)
            {
                ...
                cryozonic.stripeJsV2.source.create({
                  type: 'card',
                  token: response.id, // response, response.id, response.card.id and cardDetails throw errors
                  owner: cryozonic.getSourceOwner(),
                  redirect: {
                    return_url: null,
                  },
                },
                function(status, result)
                {
                    ...
                    if (cryozonic.getPaymentIntent())
                        cryozonic.handleCardPayment(result, done);
                    else
                        return done();
                });
            });
