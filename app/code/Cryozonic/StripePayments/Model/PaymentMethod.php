...
class PaymentMethod extends \Magento\Payment\Model\Method\Adapter
{
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        if (!empty($data['cc_saved']) && $data['cc_saved'] != 'new_card' && empty($data['cc_stripejs_token']))
        {
            ...
            if ($this->config->shouldUse3DSecure($card[0], $this->customer) && $this->helper->isAdmin())
    ...
    {
        if ($this->paymentIntent->isSuccessful())
        {
            $this->paymentIntent->associateWithOrder($payment->getOrder());
        }
        else if ($amount > 0)
        {
    ...
    public function capture(InfoInterface $payment, $amount)
    {
        if ($amount > 0)
        {
            ...
            else if ($this->paymentIntent->isSuccessful())
            {
                $this->paymentIntent->associateWithOrder($payment->getOrder());
                return $this;
            }
