...
class AttributesCommand extends Command
{
    ...
    {
        $this->setName('cryozonic:stripe-subscriptions:migrate-attributes');
        $this->setDescription('Adds the "Stripe Subscriptions" attribute group and all of its child attributes to all of the migrated attribute sets.');
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $this->addAttributeToAllAttributeSets('cryozonic_sub_enabled', 'Stripe Subscriptions', $output);
        $this->addAttributeToAllAttributeSets('cryozonic_sub_interval', 'Stripe Subscriptions', $output);
        $this->addAttributeToAllAttributeSets('cryozonic_sub_interval_count', 'Stripe Subscriptions', $output);
        $this->addAttributeToAllAttributeSets('cryozonic_sub_trial', 'Stripe Subscriptions', $output);
    }
    ...
    public function addAttributeToAllAttributeSets($attributeCode, $attributeGroupName, $output)
    {
        ...
        {
            // Add the Stripe Subscriptions group to the Attribute Set
