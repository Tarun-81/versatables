...
class Helper
{
    ...
    public function isEnabled()
    {
        ...
        $stripe_js = $this->paymentsConfig->getSecurityMethod();
        $enabled = $active && $stripe_js == '2';

        return $enabled && !$this->stripeHelper->hasSubscriptions();
    }
     ...
     * @return array
     */
    public function getBillingAddress($source)
    {
        $nameObject = $this->parseFullName($source['owner']['name']);
        $firstName = $nameObject->getFirstname();
        $lastName = $nameObject->getLastname();
        $street = [
            0 => (!empty($source['owner']['address']['line1']) ? $source['owner']['address']['line1'] : 'Unspecified Street'),
            1 => (!empty($source['owner']['address']['line2']) ? $source['owner']['address']['line2'] : '')
        ];
        $city = (!empty($source['owner']['address']['city']) ? $source['owner']['address']['city'] : 'Unspecified City');
        $region = (!empty($source['owner']['address']['state']) ? $source['owner']['address']['state'] : 'Unspecified Region');
        $postcode = (!empty($source['owner']['address']['postal_code']) ? $source['owner']['address']['postal_code'] : 'Unspecified Postcode');
        $country = (!empty($source['owner']['address']['country']) ? $source['owner']['address']['country'] : 'Unspecified Country');
